...
export default function OrderTracking() {
    ...
    // Handle auto-redirect countdown when completed
    useEffect(() => {
        let timer;
        if (order?.status === "COMPLETED" && countdown > 0) {
            timer = setTimeout(() => setCountdown(prev => prev - 1), 1000);
        } else if (order?.status === "COMPLETED" && countdown === 0) {
            navigate("/menu");
        }
    ...
    return (
        <div className="min-h-screen bg-slate-950 font-sans flex flex-col items-center">
            <div className="h-28 md:h-32 w-full flex-shrink-0" />
            <div className="w-full max-w-7xl flex justify-center px-4 md:px-12 pb-24">
                <motion.div
                    className="w-full max-w-2xl bg-white rounded-[2.5rem] p-6 md:p-10 shadow-2xl shadow-slate-950/50 border border-slate-100 relative z-10"
                ...
                >
                    ...
                    {order.status === "CANCELLED" ? (
                        ...
                        <div className="text-center p-12 bg-emerald-50 rounded-[2rem] border border-emerald-100 mb-10">
                            ...
                            <h2 className="text-emerald-900 font-black text-3xl m-0 tracking-tight">Order Completed!</h2>
                            <p className="text-emerald-700 font-bold text-sm mt-3 opacity-70">
                                Thank you for choosing Velvet Plate. We hope you enjoyed your meal!
                            </p>

                            <div className="mt-8 pt-8 border-t border-emerald-100 flex flex-col items-center gap-2">
                                <div className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Redirecting to menu in</div>
                                <div className="text-4xl font-black text-emerald-600 tabular-nums">{countdown}s</div>
                                <p className="text-xs text-emerald-600/50 font-bold mt-2 italic">order completed moving to menu</p>
                            </div>
                        </div>
                    ) : (
                    ...
                    <div className="bg-slate-50/80 rounded-[2rem] p-6 md:p-8 border border-slate-100 mb-8">
